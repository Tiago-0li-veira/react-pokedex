...
function PokeCard(props){
    ...
    return(
        ...
        onClick={() => handleCry(props.pokemon)}>
            <img 
            className="size-40" 
            src={ props.shiny ? props.pokemon.sprites.front_shiny : props.pokemon.sprites.front_default} 
            alt={props.pokemon.name}/>
            <div 
            className="flex flex-col items-center justify-center px-2 py-1 text-center bg-cyan-400 text-blue-950 border-y-2 border-blue-950 font-bold text-md">
                <div>{`#${props.pokemon.id}`}</div>
                <div>{capitalize(props.pokemon.name)}</div>
            </div>
            <div 
            className="px-2 py-2 pb-2 flex flex-row gap-2 justify-center bg-cyan-300">
